<template>
  <div class="reg bg-gray-100 h-screen">
    <div class="wrapper max-w-3xl bg-white h-full">
      <div class="inner mx-36">
        <base-header />
        <h2 class="inner__title text-6xl">Регистрация
          в личный кабинет</h2>
        <base-socials />
        <base-form>
          <template #title>
          ...
          <template #enterLogin>
            <input v-model="enteredData.name"
              class="form__input py-4 pl-5 border rounded-xl w-full text-lg mt-6 outline-none" type="text"
              placeholder="Имя и фамилия">
            <h2 v-if="!isValidName" class="error text-red-400">не верный формат имени</h2>
          </template>
          <template #enterPass>
            <input v-model="enteredData.password"
              class="form__input py-4 pl-5 border rounded-xl w-full text-lg mt-6 outline-none" type="password"
              placeholder="Пароль">
            <h2 v-if="!isValidPass" class="error text-red-400">пароль должен содержать не менее 8 символов строчные\заглавные буквы, цифры и спец символы</h2>
          </template>
          <template #enterMail>
            <input v-model="enteredData.email"
          ...
          <div @click="showCounties"
            ...
            <div v-show="isShowCountries"
              ...
              <div @click="changeCountry(country.id)" class="form__coutries flex gap-2 p-2" v-for="country of countries"
                :key="country.id" @mouseenter="activeCountry = country"
